...
--- Lab steps:
select * from  raw.airbnb.raw_listings  where id = 7071;
select * from stg.airbnb.src_listings where listing_id = 7071;
select * from int.snapshot.listings_snapshot where listing_id = 7071;  -- ['listing_url','room_type','minimum_nights','price_str'],
update  raw.airbnb.raw_listings set ROOM_TYPE  ='Entire home/apt' where id = 7071;
--now run the command "dbt build -m +listings_snapshot" -- build to run both snapshot and upstream dependent models
select * from int.snapshot.listings_snapshot where listing_id = 7071;
--Try updating minimum_nights now
update  raw.airbnb.raw_listings set minimum_nights  =3 where id = 7071;
-- run the command again - "dbt build -m +listings_snapshot" and check we have 3 rows now
select * from int.airbnb.listings_snapshot where listing_id = 7071;
--demo of invalidate_hard_deletes
-- now we will delete a row from source
select * from  raw.airbnb.raw_listings  where id = 3176;
select * from  stg.airbnb.src_listings  where listing_id = 3176;
--now we will delete this row from source 
delete from raw.airbnb.raw_listings  where id = 3176;
-- now run the dbt build command 
select * from int.snapshot.listings_snapshot  where listing_id = 3176; --it has not deleted the recrod in snapshot and the dbt_valid_to is still NULL

-- now change the configuration to invalidate_hard_deletes =True
---now run the dbt build command and check the data
-- select * from int.snapshot.listings_snapshot  where listing_id = 3176 -- the record has expired now
...
update  raw.airbnb.raw_listings set name  ='Demo room' where id = 7071;
...
select * from  raw.airbnb.raw_listings  where id = 40600;
select * from stg.airbnb.src_listings where listing_id = 40600;
